...
# Edit build.env with your OpenOCD path, etc.
...
| Script | Purpose |
|--------|---------|
| `setup_env.sh` | Installs system deps, creates Python venv, initializes Zephyr workspace, downloads SDK 0.17.0 |
| `setup_env.sh --ci` | Same as above but quieter output for CI |
| `build.sh` | Activates venv and runs `west build` for `nrf52840_mdk` |
| `build.sh --pristine` | Clean rebuild (wipes build directory first) |
| `build.sh --flash` | Builds and copies `zephyr.hex` to Windows flash directory |

### Configuration (`build.env`)

Machine-specific paths live in `build.env` (gitignored). Copy the template and edit:
```bash
cp build.env.example build.env
```
...
cd <ZEPHYR_WORKSPACE>
west build -b nrf52840_mdk -d <project>/app/build <project>/app
```
...
./build.sh --flash
```
This copies `app/build/zephyr/zephyr.hex` → `C:/nrf_recovery/app.hex`.
